...
class ResumeAnalysis(models.Model):
    ...
    def score(self):
        if self.report.get('overall_score') is not None and self.report.get('overall_score') >= 50:
            return 'high'
        elif self.report.get('overall_score') is not None and self.report.get('overall_score') >= 21:
            return 'medium'
        else:
            return 'low'